...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
	...
	public function deleteProduct($product_id) {
                ...
                $qr = $this->db->query("SELECT sku FROM ".DB_PREFIX."product WHERE product_id = '".$product_id."' ");
                $ar = $this->db->query("SELECT * FROM ".DB_PREFIX."complects WHERE heading = '".$qr->row['sku']."' ");
                if(!empty($ar->row)){
                    $this->db->query("UPDATE ".DB_PREFIX."product SET comp = '' WHERE comp = '".$qr->row['sku']."' ");
                    $this->db->query("DELETE FROM " . DB_PREFIX . "complects WHERE heading = '" . $qr->row['sku'] . "'");
                }
