	...
	public function info() {
		...
		if (isset($this->request->get['manufacturer_id'])) {
			...
			$manufacturer_id = 0;
                ...
                $results = $this->model_catalog_manufacturer->getModsNRs($manufacturer_id);
                
                foreach ($results as $result) {
                    
                    $filter_data = array(
			'filter_model_id'  => $result['id'],
                        'filter_sub_category' => true
                    );

                    $data['models'][] = array(
			'name' => $result['name'] . ($this->config->get('config_product_count') ? ' (' . $this->model_catalog_product->getTotalProducts($filter_data) . ')' : ''),
                        'href' => $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . '_' . $result['id'])
                    );
		}
